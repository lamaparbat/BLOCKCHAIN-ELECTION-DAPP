...
const Details: React.FC = (): React.ReactElement => {
  ...
  const selectedCandidates = useSelector((state: any) => state.candidateReducer);
  const { partyName, agenda, voteCount, user }: any | undefined = selectedCandidates.details;
  const {
    fullName,
    dob,
    age,
    ...
    return (
      <div className='flex justify-between mb-4'>
        ...
        <div className='text-end'>
          <h6>{title2}:</h6>
          <span className='relative'>{title2Val}</span>
        </div>
      </div>
    )
  }
      ...
      <div className='w-full flex justify-center'>
        <div className={`${responsive} flex justify-between rounded-1 flex-wrap`}>
          <div className='bg-slate-100 w-[550px] h-[375px] mb-3 flex justify-center items-center overflow-hidden rounded-2 border-2 border-slate-300'>
            {true ? <img src={profile} alt={fullName} height="100%" width="100%" /> : <FaUserCircle className='text-3xl text-slate-500' />}
          </div>
          <div className='details w-[500px]'>
            <FieldRow title1="Candidate Name" title1Val={fullName} title2="Date of birth (DOB)" title2Val={moment(dob).format("LLL")} />
            <FieldRow title1="Birth Place" title1Val={`${municipality}, ${district}`} title2="Temporary Address" title2Val="Kathmandu Nepal" />
